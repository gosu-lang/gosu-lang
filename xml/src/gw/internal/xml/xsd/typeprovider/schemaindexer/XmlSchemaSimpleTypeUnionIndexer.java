...
public class XmlSchemaSimpleTypeUnionIndexer extends XmlSchemaIndexer<XmlSchemaSimpleTypeUnion> {
  @Override
  public <T> void index( XmlSchemaSimpleTypeUnion union, String prefix, List<XmlSchemaFlattenedChild> flattenedChildren, boolean topLevel, boolean isPlural, XmlSchemaType currentType, boolean createTypes, Map<QName, XmlSchemaElementTypeData> usedElementNames, Map<Pair<XmlSchemaPropertyType, QName>, Boolean> pluralityMap, T context, List<Runnable> todo ) {
    for ( XmlSchemaSimpleType simpleType : union.getBaseTypes() ) {
      invokeIndexer( simpleType, prefix, flattenedChildren, topLevel, isPlural, currentType, createTypes, usedElementNames, pluralityMap, context, todo );
    }
  }
