...
class MoveClassTest extends GosuTestCase
{

  //TestClass1 will be moved to some.pkg2, and TestClass2 will be modified accordingly
  function testMoveExtendClassFromTheSamePackage() {
  ...
  function testMoveExtendClassFromDifferentPackageToAThirdPackage() {
    test("some.one.pkg3", "some.one.pkg3",{
        ...
        "package some.one.pkg2\n" +
            ...
            "cla^^ss TestClass2 extends TestClass1 {\n" +
            "}"
    })
  }

  function testMoveInterfaceFromSamePackageToDifferentOne() {
    test("some.one.pkg2", "\nuses some.one.pkg2.ITestClass1\n\n",{
        "package [[some.one.pkg]]\n" +
        ...
        "package some.one.pkg\n" +
            "[[]]" +
            "class TestClass2 implements ITestClass1 {\n" +
            "}"
    })
  }

  function testMoveInterfaceFromDifferentPackageToAThirdPackage() {
    test("some.one.pkg3", "some.one.pkg3", {
        "package [[some.one.pkg1]]\n" +
            "interface ITestClass1 {\n" +
            "}",
        "package some.one.pkg3\n" +
            ...
            "}",
        "package some.one.pkg2\n" +
            "uses [[some.one.pkg1]].ITestClass1\n" +
            "class TestClass2 implements ITestClass1 {\n" +
            "}"
    })
  }
  ...
  function testImpl(newPackage: String, newUses: String, files: List <GosuTestingResource>) {
    ...
    var gsClassFile3 = (psiFiles.get(2) as PsiFileImpl).calcTreeElement().Psi as GosuClassFileImpl
