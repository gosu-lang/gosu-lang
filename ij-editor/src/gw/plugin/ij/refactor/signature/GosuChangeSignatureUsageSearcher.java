...
class GosuChangeSignatureUsageSearcher {
  private final JavaChangeInfo myChangeInfo;
  private static final Logger LOG = Logger.getInstance("#com.intellij.refactoring.changeSignature.GosuChangeSignatureUsageSearcher");

  GosuChangeSignatureUsageSearcher(JavaChangeInfo changeInfo) {
  ...
  private void findUsagesInCallers(final ArrayList<UsageInfo> usages) {
    if (myChangeInfo instanceof JavaChangeInfo) {
      ...
      for (final PsiMethod method : merged) {
        findSimpleUsagesWithoutParameters(method, usages, changeInfo.getMethodsToPropagateParameters().contains(method),
                                          false, false);
      }
    }
  }
  ...
  private static boolean shouldPropagateToNonPhysicalMethod(PsiMethod method,
                                                            ...
                                                            final Collection<PsiMethod> propagateMethods) {
