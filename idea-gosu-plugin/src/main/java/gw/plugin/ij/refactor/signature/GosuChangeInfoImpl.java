...
class GosuChangeInfoImpl implements JavaChangeInfo {
   ...
   */
  public GosuChangeInfoImpl(@PsiModifier.ModifierConstant String newVisibility,
                            PsiMethod method,
                            String newName,
  ...
  public GosuChangeInfoImpl(@PsiModifier.ModifierConstant String newVisibility,
    ...
    if (newType == null) {
      newType = CanonicalTypes.createTypeWrapper(PsiType.VOID);
    }
    ...
    this.propagateExceptionsMethods=propagateExceptionsMethods == null ? Collections.<PsiMethod>emptySet() : propagateExceptionsMethods;
    this.propagateParametersMethods=propagateParametersMethods == null ? Collections.<PsiMethod>emptySet() : propagateParametersMethods;
    ...
    for(int i = 0; i < newParms.length; i++){
      ...
      if (info.oldParameterIndex < 0 && !info.isVarargType()){
        if (StringUtil.isEmpty(info.getDefaultValue())) continue;
        try{
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof GosuChangeInfoImpl)) return false;

    GosuChangeInfoImpl that = (GosuChangeInfoImpl)o;
