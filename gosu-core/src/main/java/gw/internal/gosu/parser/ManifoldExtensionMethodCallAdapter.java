...
public class ManifoldExtensionMethodCallAdapter implements IMethodCallHandler
{
  ...
  public ManifoldExtensionMethodCallAdapter( JavaMethodInfo mi )
  {
    ...
    extType.getTypeInfo().getMethod( mi.getName(), paramTypes.toArray( new IType[paramTypes.size()] ) );
    IMethodInfo extMi = extType.getTypeInfo().getMethod( mi.getDisplayName(), paramTypes.toArray( new IType[paramTypes.size()] ) );
    if( extMi == null )
    {
